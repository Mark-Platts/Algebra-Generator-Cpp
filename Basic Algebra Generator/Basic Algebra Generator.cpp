...
std::array<int, 6> gen_vals_1(const std::array<double, 4>& const_probs, const std::array<double, 4>& linear_probs) {
    ...
    if (hold[1] == 0 && hold[2] == 0) {
        hold[3] = hold[0] * randint_wn(1, 10);
    }
    return hold;
}
...
int algebra_loop(double type_prob, const std::array<double, 4>& const_probs, const std::array<double, 4>& linear_probs) {
    ...
    while (true) {
        ...
        std::string x;
        std::cin >> x;
        if (x == std::string(1,'n') || x == std::string(1, 'N')) {
            break;
        }
        else if (x == std::string(1, 'l') || x == std::string(1,'L')) {
            return(0);
        }
        else {
    ...
    while (true) {
        ...
        else if (y != ans && y != std::string(1,'s') && y != std::string(1,'S') && y != std::string(1,'d') && y != std::string(1,'D')) {
            std::cout << "Sorry! Try again or type 's' to see the solution." << std::endl;
        }
        else if (y == std::string(1,'s') || y == std::string(1,'S')) {
            std::cout << std::string(80, '-') << std::endl; //\u2550
            std::cout << sol << std::endl;
            break;
        }
        else if (y == std::string(1,'d') || y == std::string(1,'D')) {
            print_equ(equ);
...
//A loop function that allows selection of level and initializes appropriate alg_loop
void level_loop() {
    std::array<double, 5> type_probs_by_level{ 1, 1, 0.5, 0.3, 0 };
    double const_probs_by_level[5][4]
    {
        { 0.2, 0.5, 0, 0 },
        { 0.2, 0.2, 0, 0 },
        { 0.5, 1, 0, 0.5 },
    ...
    double linear_probs_by_level[5][4]
    {
        { 0, 0.5, 1, 0 },
        { 0, 0.1, 0.2, 0.5 },
        { 0.5, 0.5, 1, 0 },
    ...
    std::string x;
    while (true) {
        std::cout << "Please type a number from 1-5 to select a level." << std::endl;
        std::cin >> x;

        if (x == std::string(1,'1') || x == std::string(1, '2') || x == std::string(1, '3') || x == std::string(1, '4') || x == std::string(1, '5')) {
            break;
        }
        else {
            std::cout << "Command unknown." << std::endl;
    ...
    double type_prob = type_probs_by_level[std::stoi(x) - 1];
    std::array<double, 4> const_probs{ 0.0 };
    std::array<double, 4> linear_probs{ 0.0 };
    for (int i = 0; i < 4; ++i) {
        const_probs[i] = const_probs_by_level[std::stoi(x) - 1][i];
        linear_probs[i] = linear_probs_by_level[std::stoi(x) - 1][i];
    }
    int B = 1;
    while (B == 1) {
        B = algebra_loop(type_prob, const_probs, linear_probs);
    }
}
